...
Version 0.1.5 establishes a ten-agent organization covering direction, product and PRD review, architecture, implementation support, independent code review, incident response, end-to-end validation, brand and UI quality, and public documentation. The organization combines hub-and-spoke intake with explicit pipelines for features, pull requests, incidents, releases, and documentation drift. Every agent also shares a durable-completion contract that prevents duplicate final reports, unsourced execution timestamps, unbounded context expansion, and ambiguous aggregation of workflow, report, object, readiness, and evidence states. It records the blocked migration from legacy Landlock to Bubblewrap and prevents a warning-only change from silently disabling agent command execution or weakening the shared control-plane container. It also records Linear team `OPT` as the Board-approved operational backlog authority and constrains the initial connection to Linear's official read-only MCP endpoint for Product only.

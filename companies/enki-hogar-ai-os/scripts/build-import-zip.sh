...
if [ -e "$output.sha256" ]; then
  echo "Refusing to overwrite existing checksum: $output.sha256" >&2
  exit 3
fi
...
case "$output_path" in
  "$package_dir"/*)
    echo "Output must be outside the source package" >&2
    exit 5
    ;;
esac
...
while IFS= read -r required; do
  ...
  if ! grep -F -x "$required" "$file_list" >/dev/null; then
    echo "Missing required import file: $required" >&2
    exit 6
  fi
done < "$allowlist"
...
# ZIP stores timestamps and Unix mode bits. Normalize both, use a sorted file
# list, and store without compression so zlib-version differences cannot alter
# the artifact bytes.
...
find "$archive_root/connectors" -type f -name '*.sh' -exec chmod 0755 {} +
find "$archive_root/skills/wordpress-publisher/scripts" -type f -name '*.py' -exec chmod 0755 {} +
(
  cd "$archive_root"

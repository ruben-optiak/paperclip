...
- Four initial projects, twenty-seven bootstrap tasks, and two paused routines.
- Fourteen domain skills with examples and offline fixtures, including decision-gated editorial planning, governed product drafts, PII-bounded proformas and human-governed learning.
- Curated, non-secret Enki knowledge with an allowlisted sync process.
- A pinned, networkless catalogue runtime with a small geometry core, four snapshot-scoped brand adapters, strict positional Woo reconciliation, deterministic WebP preparation, idempotent local change sets and post-import drift audit; immutable sanitized regressions; a closed read-only MCP for Board-approved run/field evidence; a read-only WooCommerce MCP with live parent/variation inspection; pinned Google MCP runtime; audited Telegram gateway plugin; isolated PostgreSQL/pgvector product-support projection; and a governed WordPress/WooCommerce-draft/Facebook/Instagram publisher.
- Desired connection policy, six agent-scoped managed gateways, security controls, tests, and promotion runbooks.
- A bounded [SEO pipeline](runbooks/seo-pipeline.md), dated sitemap/HTML evidence, source authentication probes and offline technical/search prioritization. The restored Google capture retains 35 search review opportunities, partial query overlap and 19 index observations, alongside the immutable initial authentication failure receipt.
- Dated, read-only Merchant and GSC follow-ups: the current Merchant diagnosis and complete 25-offer rejection audit, plus the warning-counter, Search Console UI and public HTTP receipts for the unreadable brand sitemap. These artifacts preserve unresolved causes and require separate approval for every proposed external correction.
...
| Ecommerce & Catalogue Manager | Director | catalogue QA and technical product support, governed product drafts, brand guardian, editorial candidate validation, daily brief, change control |
...
| Customer Experience Manager | Director | customer care, technical product facts, PII-bounded proformas, brand guardian, change control |
...
1. Copy `.env.example` to an untracked environment file outside Git, generate the independent Paperclip tool-action signing secret with the provided helper, and fill only connector-side credentials. Keep both editorial/social and product publishing in `disabled` mode until their separate smoke gates pass. Leave all embedding fields empty unless intentionally configured. Store the Telegram token as a Paperclip Secret, never in `.env`.
...
4. Build the import archive with `./companies/enki-hogar-ai-os/scripts/build-import-zip.sh /tmp/enki-hogar-ai-os-v0.17.0.zip` and preview that exact ZIP with the current Paperclip CLI or UI before applying it.
5. Keep all agents and routines paused while configuring connections and the six disabled agent-scoped gateways; never use connection installs for Enki. Reconcile the publisher with `scripts/reconcile-content-publisher.mjs --apply` only while both independent write modes are `disabled`.

For a new disposable company, preview the generated ZIP rather than the source directory:

```sh
npx paperclipai company import /tmp/enki-hogar-ai-os-v0.17.0.zip \
  --target new \
...
```sh
pnpm paperclipai company import /tmp/enki-hogar-ai-os-v0.17.0.zip \
...
Allowed in v1: approved reads, analysis, comparisons, evidence packs, local drafts, one local proforma transform and three narrowly governed publication calls. Brand Guardian reviews customer-facing drafts but grants no publication authority. Growth may request a WordPress post upsert, a Facebook Page post or one Instagram image only after the exact draft/review handoff; Paperclip asks Board and the connector enforces its own kill switch plus idempotency journal. Customer and exact-order lookup remain absent. Customer Experience may use PII only from one Board-provided private proforma request, keep it out of Paperclip/Git/logs/receipts, and produce no message or external write. See [proformas](runbooks/proformas.md), [access matrix](policies/access-matrix.md) and [change control](skills/enki-change-control/SKILL.md).

...
test("Compose binds host health ports to loopback and never injects upstream credentials into agents", () => {
  ...
  for (const port of [8010, 8011, 8012, 8020]) assert.match(compose, new RegExp(`127\\.0\\.0\\.1:\\$\\{[^}]+:-${port}}:${port}`));
  ...
  assert.doesNotMatch(agentConfig, /WOO_CONSUMER|GOOGLE_ADS_DEVELOPER_TOKEN|GOOGLE_CLIENT_SECRET/);
});
...
test("operator docs keep the complete raw ZIP UI path separate from the partial CLI preview", () => {
  ...
  assert.match(readme, /only supported apply path[\s\S]*generated ZIP[\s\S]*Paperclip\s+UI/i);
  assert.match(readme, /Keep `--dry-run`[\s\S]*must not be applied/i);
  assert.doesNotMatch(readme, /equivalent CLI entrypoint/i);
  assert.match(localSetup, /apply imports only[\s\S]*raw ZIP[\s\S]*Paperclip\s+UI/i);
  assert.match(localSetup, /Do not apply[\s\S]*paperclipai company import/i);
  assert.match(promotion, /raw ZIP through the Paperclip\s+UI/i);
});

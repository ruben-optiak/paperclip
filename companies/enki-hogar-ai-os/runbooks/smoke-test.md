...
9. Publishing connector: with both kill switches `disabled`, `publisher_get_capabilities` reports all unconfigured providers without IDs or secrets; the remaining reads, including `wordpress_get_article` and the two mounted product-bundle reads, either return bounded data or a clear `not configured` result, and all six write tools fail before mutation. Verify the observed catalog is exactly fourteen tools and the six writes are classified write/non-destructive/idempotent. Simulate all six policy decisions through `/tools/policy/test` and require `decision=require_approval` without creating an action request.

...
Treat these as four different artifacts. None replaces another:
...
4. **Support-projection backup**: a consistent `pg_dump` of the isolated Enki product-support database plus the source-repository revision/checksum of every active approved pack. It is rebuildable from those packs, but a company export and Paperclip instance backup do not contain this volume.
...
- take Paperclip database/files and support-projection backups at documented recovery points;
...
A backup is accepted only after a restore drill in an isolated target proves both database startups, support schema/coverage, file availability, company visibility, authentication, and a paused-agent smoke test. Record redacted evidence and recovery time. Rollback restores Paperclip database/files and the compatible support dump (or rebuilds it from pinned packs), redeploys the previous immutable images, re-applies manually managed connections/policies if needed, and keeps all agents and schedules paused until smoke tests pass.

...
function createMcp(dependencies) {
  const server = new McpServer({name: "enki-content-publisher", version: CONNECTOR_VERSION});
  for (const tool of createToolDefinitions(dependencies)) {
...
export function createHttpServer({dependencies, token}) {
  return createServer((request, response) => {
    void (async () => {
      const url = new URL(request.url || "/", "http://localhost");
      if (request.method === "GET" && url.pathname === "/health") {
        sendJson(response, 200, {status: "ok", service: "enki-content-publisher-mcp", version: CONNECTOR_VERSION});
        return;
      }
      if (url.pathname !== "/mcp") {
      ...
      const mcp = createMcp(dependencies);
      const transport = new StreamableHTTPServerTransport({sessionIdGenerator: undefined});
      response.on("close", () => { void transport.close(); void mcp.close(); });
      await mcp.connect(transport);
      await transport.handleRequest(request, response, body);
    })().catch(() => {
      if (!response.headersSent) sendJson(response, 500, {error: "Connector request failed"});
      else response.end();
    });
  });
}

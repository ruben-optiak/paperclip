...
- `OAI-013` está representado por [OPT-30](/OPT/issues/OPT-30). El inventario local encontró repositorios separados de producto, frontend, pruebas, ML e infraestructura. La propuesta mínima pendiente de Board limita el primer smoke a `optiak/optiak`, `optiak/optiak-frontend` y `optiak/optiak-tests`, mediante credencial GitHub fine-grained read-only y visibilidad inicial exclusiva del Independent Reviewer. No se ha creado credencial ni conexión.
...
- El backlog de producto tiene una conexión Linear autenticada y validada en lectura; no se ha copiado su contenido a Paperclip. Git/PRs, repositorios, staging, observabilidad y producción continúan desconectados. Ningún resultado fixture se considera evidencia live.

## Orden inmediato

1. Continuar con `OAI-013` seleccionando los repositorios exactos y el alcance read-only de PRs/checks.
...
| `OAI-013` | P1 | BLOCKED | Lectura de repositorios, PRs y checks | Resolver en [OPT-30](/OPT/issues/OPT-30) la allowlist exacta, el tipo de credencial y los agentes iniciales; después versionar el contrato antes de conectar | Lectura positiva, denegación de merge/write, revisión por SHA, redacción, revocación y auditoría comprobadas |

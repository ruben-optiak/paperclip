...
Paquete actual: `companies/optiak-ai-os/` (`0.1.6`)
...
1. Continuar con `OAI-013` creando el PAT fine-grained exacto, conectándolo solo al Reviewer y ejecutando el smoke por SHA.
2. Preparar `OAI-014` solo cuando exista un tenant de staging sintético, aislado y sin alcance de producción.
3. Retomar `OAI-011` únicamente cuando exista un boundary de ejecución dedicado.
...
| `OAI-013` | P1 | BLOCKED | Lectura de repositorios, PRs y checks | Crear una credencial GitHub fine-grained limitada a `optiak/optiak` y `optiak/optiak-frontend`, conectar el endpoint read-only solo al Reviewer y ejecutar el smoke de [OPT-30](/OPT/issues/OPT-30) | Lectura positiva de ambos repos, denegación por política de todo repo no listado, ausencia de merge/write, revisión por SHA, redacción, revocación y auditoría comprobadas |

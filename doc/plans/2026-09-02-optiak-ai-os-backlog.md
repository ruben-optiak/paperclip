...
1. Ejecutar `OAI-010`, los smokes acotados de product triage, API conformance y change control.
2. Resolver `OAI-021`, evitando comentarios duplicados y payloads temporales en el cierre durable.
3. Ejecutar `OAI-016`, la síntesis fixture-only del Director, cuando `OAI-010` y `OAI-021` hayan pasado.
4. Medir y reducir contexto con `OAI-018`, y retirar el sandbox legacy mediante `OAI-011` solo tras su regresión.
...
| `OAI-010` | P1 | NOW | Cobertura de skills transversales | Diseñar smokes acotados para product triage, API conformance y change control | Cada una tiene resultado esperado, caso negativo, evidencia durable y cero afirmaciones live |
...
| `OAI-016` | P1 | BLOCKED | Smoke de síntesis del Director | Esperar a `OAI-010` y `OAI-021`; solicitar una priorización basada solo en evidencias de los smokes | El Director cita fuentes, separa fixture de live, no crea autoridad nueva y devuelve owners y decisiones acotadas |
...
| `OAI-021` | P1 | READY | Cierre durable idempotente | Reproducir el doble comentario de [OPT-3](/OPT/issues/OPT-3) y el bloqueo previo a API de [OPT-23](/OPT/issues/OPT-23); estandarizar un único payload en memoria | Una run escribe exactamente un informe, marca el issue una vez, no usa archivos temporales y conserva la trazabilidad del run |

## Dependencias principales

- Validación fixture restante y cierre fiable: `OAI-010` + `OAI-021` → `OAI-016`.
...
- 2026-09-02 — Baseline `OAI-018` para `OAI-005`–`OAI-009`: 731815 tokens de entrada, 620032 cacheados y 19340 de salida. El rango por run fue 92446–211045 tokens de entrada, suficiente para priorizar reducción de contexto antes de rutinas.

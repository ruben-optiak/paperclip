...
- Paperclip keeps authentication, managed MCP configuration, and session state in the isolated CODEX_HOME. For local managed-heartbeat runs it materializes desired skills under the run-owned temporary HOME at .agents/skills, which Codex discovers and a read-only sandbox can read without gaining access to CODEX_HOME credentials; the temporary HOME is removed with run scratch at terminal disposition. Remote managed homes keep materialized skills in the staged CODEX_HOME, while an explicitly overridden external CODEX_HOME retains the legacy link behavior so Paperclip does not overwrite user-managed skill directories.

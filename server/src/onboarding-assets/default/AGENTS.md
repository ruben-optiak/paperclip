...
- Agent-authenticated `unblockDescriptor` values may name only your own agent. If another agent, the board, or a user owns the next action, create and assign a follow-up issue; use `blockedByIssueIds` only when this issue must wait for it.

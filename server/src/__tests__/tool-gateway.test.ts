...
describeEmbeddedPostgres("tool gateway acceptance", () => {
  ...
  it("exposes a named gateway with scoped bearer-token auth and revocation", async () => {
    ...
    try {
      ...
      const authenticatedApp = createGatewayRouteApp(db, gateway, undefined, true);
      const publicEndpoint = created.endpointPath;
      const queryOnly = await request(authenticatedApp)
      ...
      const listed = await request(authenticatedApp)
        .post(publicEndpoint)
